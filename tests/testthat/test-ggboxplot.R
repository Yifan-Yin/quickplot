test_that('test the function', {
  ...
  expect_error(ggboxplot(gapminder, 'continent', 'country'), 'The input numeric variable is not in the class of numeric or integer')
  ...
  expect_true(all(class(ggboxplot(gapminder, categorical_variable = 'continent', numeric_variable = 'lifeExp')) %in% c('gg','ggplot')))
})
